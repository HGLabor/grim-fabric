...
public class SetbackTeleportUtil extends Check implements PostPredictionCheck {
    ...
    public void executeForceResync() {
        ...
        if (lastKnownGoodPosition == null) return; // ServerPlayerEntity hasn't spawned yet
    ...
    private boolean isExempt() {
        ...
        // ServerPlayerEntity hasn't spawned yet
        if (lastKnownGoodPosition == null) return true;
        // Setbacks aren't allowed
        if (player.disableGrim) return true;
        // ServerPlayerEntity has permission to cheat, permission not given to OP by default.
    ...
    private void sendSetback(SetBackData data) {
        ...
        try {
            // ServerPlayerEntity is in a vehicle
            if (player.inVehicle()) {
            ...
            addSentTeleport(new Vector3d(position.getX(), y, position.getZ()/*TODO, player.xRot % 360, player.yRot % 360*/), data.getTeleportData().getTransaction(), new RelativeFlag(0b11000), false, teleportId);
    ...
    /**
     * @param x - ServerPlayerEntity X position
     * @param y - ServerPlayerEntity Y position
     * @param z - ServerPlayerEntity Z position
    ...
    public TeleportAcceptData checkTeleportQueue(double x, double y, double z) {
        ...
        while ((teleportPos = pendingTeleports.peek()) != null) {
            ...
            if (player.lastTransactionReceived.get() == teleportPos.getTransaction() && Math.abs(clamped.getX() - x) <= threshold && closeEnoughY && Math.abs(clamped.getZ() - z) <= threshold) {
                ...
                // ServerPlayerEntity has accepted their setback!
    ...
    public void addSentTeleport(Vector3d position, int transaction, RelativeFlag flags, boolean plugin, int teleportId) {
