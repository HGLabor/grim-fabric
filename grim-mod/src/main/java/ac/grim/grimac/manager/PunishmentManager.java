            ...
            if (group.checks.contains(check)) {
                ...
                for (ParsedCommand command : group.commands) {
                    ...
                    if (!GrimAPI.INSTANCE.getAlertManager().getEnabledVerbose().isEmpty() && command.command.equals("[alert]")) {
                        sentDebug = true;
                        for (ServerPlayerEntity bukkitPlayer : GrimAPI.INSTANCE.getAlertManager().getEnabledVerbose()) {
                            MessageUtil.sendMessage(bukkitPlayer.getCommandSource(), MessageUtil.miniMessage(cmd));
                        }
                        if (printToConsole) {
                    ...
                    if (violationCount >= command.threshold) {
                        ...
                        if (inInterval) {
                            ...
                            AntiCheatEvents.INSTANCE.getCommandExecuteEvent().invoke(executeEvent);
                            if (executeEvent.isCancelled().get()) continue;
                            ...
                            if (command.command.equals("[webhook]")) {
                                ...
                                SchedulerUtils.INSTANCE.sync(() -> {
                                    var server = GrimAPI.INSTANCE.getServer();
                                    var parsed = server.getCommandManager().getDispatcher().parse(finalCmd, server.getCommandSource());
                                    GrimAPI.INSTANCE.getServer().getCommandManager().execute(parsed, finalCmd);
                                });
                            }
                        }
