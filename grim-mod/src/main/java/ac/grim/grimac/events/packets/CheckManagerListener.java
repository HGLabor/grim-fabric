    ...
    public void onPacketReceive(PacketReceiveEvent event) {
        ...
        if (event.getPacketType() == PacketType.Play.Client.PLAYER_DIGGING) {
            ...
            if (action == DiggingAction.START_DIGGING || action == DiggingAction.FINISHED_DIGGING || action == DiggingAction.CANCELLED_DIGGING) {
                final BlockBreak blockBreak = new BlockBreak(player, packet.getBlockPosition(), packet.getBlockFace(), packet.getBlockFace().getFaceValue(), action, player.compensatedWorld.getBlock(packet.getBlockPosition()));
                ...
                if (event.isCancelled() || blockPlace.isCancelled() || player.getSetbackTeleportUtil().shouldBlockMovement()) { // The player tried placing blocks in air/water
                    ...
                    if (player.bukkitPlayer != null) {
                        if (packet.getHand() == InteractionHand.MAIN_HAND) {
                            ItemStack mainHand = SpigotConversionUtil.fromBukkitItemStack(player.bukkitPlayer.getMainHandStack());
                            ...
                            ItemStack offHand = SpigotConversionUtil.fromBukkitItemStack(player.bukkitPlayer.getOffHandStack());
