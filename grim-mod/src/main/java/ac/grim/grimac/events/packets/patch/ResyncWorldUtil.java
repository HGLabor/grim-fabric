...
public class ResyncWorldUtil {
    static HashMap<Block, Integer> blockDataToId = new HashMap<>();
    ...
    public static void resyncPositions(GrimPlayer player, int minBlockX, int mY, int minBlockZ, int maxBlockX, int mxY, int maxBlockZ) {
        ...
        SchedulerUtils.INSTANCE.sync(() -> {
            ...
            // ServerPlayerEntity hasn't spawned, don't spam packets
            ...
            for (int currChunkZ = minChunkZ; currChunkZ <= maxChunkZ; ++currChunkZ) {
                ...
                for (int currChunkX = minChunkX; currChunkX <= maxChunkX; ++currChunkX) {
                    ...
                    Chunk chunk = world.getChunk(currChunkX, currChunkZ);
                        ...
                        for (int currZ = minZ; currZ <= maxZ; ++currZ) {
                            for (int currX = minX; currX <= maxX; ++currX) {
                                for (int currY = minY; currY <= maxY; ++currY) {
                                    var block = chunk.getBlockState(new BlockPos(currX, currY | (currChunkY << 4), currZ));

                                    Identifier id = Registries.BLOCK.getId(block.getBlock());
                                    int blockId = blockDataToId.computeIfAbsent(block.getBlock(), data -> WrappedBlockState.getByString(PacketEvents.getAPI().getServerManager().getVersion().toClientVersion(), id.toString()).getGlobalId());
    ...
    public static void resyncPosition(GrimPlayer player, Vector3i pos, int sequence) {
        ...
        SchedulerUtils.INSTANCE.sync(() -> {
            if (player.bukkitPlayer.isDisconnected() || !player.getSetbackTeleportUtil().hasAcceptedSpawnTeleport)
                return;

            if (!player.compensatedWorld.isChunkLoaded(chunkX, chunkZ)) return;
            if (player.bukkitPlayer.getPos().distanceTo(new Vec3d(pos.x, pos.y, pos.z)) >= 64) return;
            if (!world.isChunkLoaded(chunkX, chunkZ)) return; // Don't load chunks sync

            final Block block = world.getChunk(chunkX, chunkZ).getBlockState(new BlockPos(pos.x & 15, pos.y, pos.z & 15)).getBlock();
            Identifier id = Registries.BLOCK.getId(block);

            final int blockId = blockDataToId.computeIfAbsent(block, data -> WrappedBlockState.getByString(PacketEvents.getAPI().getServerManager().getVersion().toClientVersion(), id.toString()).getGlobalId());
