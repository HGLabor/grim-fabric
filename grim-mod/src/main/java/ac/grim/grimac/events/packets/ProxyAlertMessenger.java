...
public class ProxyAlertMessenger extends PacketListenerAbstract {
    ...
    public ProxyAlertMessenger() {
        ...
        if (usingProxy) {
            ...
            //TODO GrimAPI.INSTANCE.getServer().getMessenger().registerOutgoingPluginChannel(GrimAPI.INSTANCE.getServer(), "BungeeCord");
        }
    }
    ...
    public void onPacketReceive(final PacketReceiveEvent event) {
        ...
        for (ServerPlayerEntity bukkitPlayer : GrimAPI.INSTANCE.getAlertManager().getEnabledAlerts())
            MessageUtil.sendMessage(bukkitPlayer.getCommandSource(), MessageUtil.miniMessage(alert));
    }
    ...
    public static void sendPluginMessage(String message) {
        ...
        //TODO Iterables.getFirst(Bukkit.getOnlinePlayers(), null).sendPluginMessage(GrimAPI.INSTANCE.getServer(), "BungeeCord", out.toByteArray());
    }

    public static boolean canSendAlerts() {
        return usingProxy && GrimAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("alerts.proxy.send", false) && !GrimAPI.INSTANCE.getServer().getPlayerManager().getPlayerList().isEmpty();
    }
    ...
    private static boolean getBooleanFromFile(String pathToFile, String pathToValue) {
        ...
        return false;
        //TODO return YamlConfiguration.loadConfiguration(file).getBoolean(pathToValue);
    }
}
