...
public class DebugHandler extends Check implements PostPredictionCheck {
    ...
    Set<ServerPlayerEntity> listeners = new CopyOnWriteArraySet<>(new HashSet<>());
    ...
    public void onPredictionComplete(final PredictionComplete predictionComplete) {
        ...
        for (ServerPlayerEntity player : listeners) {
            ...
            player.sendMessage(Text.of((player == getPlayer().bukkitPlayer ? "" : prefix) + p));
            player.sendMessage(Text.of((player == getPlayer().bukkitPlayer ? "" : prefix) + a));
            player.sendMessage(Text.of((player == getPlayer().bukkitPlayer ? "" : prefix) + o));
        }

        // Don't memory leak player references
        listeners.removeIf(player -> player.isDisconnected());
    ...
    public void toggleListener(ServerPlayerEntity player) {
