...
public class CorrectingPlayerInventoryStorage extends InventoryStorage {
    ...
    private void checkThatBukkitIsSynced(int slot) {
        ...
        int bukkitSlot = slot; //TODO?player.getInventory().getBukkitSlot(slot); // 8 -> 39, should be 36

        if (bukkitSlot != -1) {
            var bukkitItem = player.bukkitPlayer.getInventory().getStack(bukkitSlot);
            ...
            if (existing.getType() != toPE.getType() || existing.getAmount() != toPE.getAmount()) {
                SchedulerUtils.INSTANCE.sync(() -> {
                    player.bukkitPlayer.getInventory().updateItems();
                });
                setItem(slot, toPE);
            }
        }
    ...
    public void tickWithBukkit() {
        ...
        if (player.getInventory().needResend) {
            SchedulerUtils.INSTANCE.sync(() -> {
                ...
                /*TODO InventoryView view = player.bukkitPlayer.playerScreenHandler.getType();
